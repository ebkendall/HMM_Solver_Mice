...
sub_ind = sample(unique(mice_format$ptnum), 50)
...
n_post = 5000; steps2 = 10000
index_post = (steps2 - burnin - n_post + 1):(steps2 - burnin)
load(paste0('Model_out/mcmc_out_', ind, '_', trialNum - 1,'.rda'))
init_par = pars = colMeans(mcmc_out$chain[index_post,])
rownames(pars) = NULL
