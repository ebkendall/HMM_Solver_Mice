...
Q <- function(t,x_ik,beta){
    ...
    q9  = exp( c(1,t,x_ik) %*% betaMat[9,] )  # Transition from REM   to AWAKE
    q10 = exp( c(1,t,x_ik) %*% betaMat[10,] ) # Transition from REM   to IS
    q11 = exp( c(1,t,x_ik) %*% betaMat[11,] ) # Transition from REM   to NREM
